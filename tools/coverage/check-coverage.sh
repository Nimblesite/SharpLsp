...
#
# Reads the threshold from .config/coverage/thresholds.json for the given project key.
...
TEMP_THRESHOLDS="${THRESHOLDS}.tmp"
...
if [ "$ABOVE" -eq 1 ]; then
  ...
  if [ "$RATCHET_OK" -eq 1 ]; then
    ...
    jq --arg p "$PROJECT" --argjson new "$NEW_THRESHOLD" '.[$p].line_percent = $new' "$THRESHOLDS" > "$TEMP_THRESHOLDS"
    mv "$TEMP_THRESHOLDS" "$THRESHOLDS"
  fi
fi
