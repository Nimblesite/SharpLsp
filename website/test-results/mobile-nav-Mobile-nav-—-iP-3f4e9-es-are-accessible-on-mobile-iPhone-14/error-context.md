...
- Name: mobile-nav.spec.js >> Mobile nav — iPhone 14 (390px) >> all new docs pages are accessible on mobile
- Location: tests/mobile-nav.spec.js:98:5

# Error details

```
Error: /forge/docs/ should return 200
...
```yaml
- generic [ref=e2]: Cannot GET /forge/docs/
```
...
> 112 |         expect(response?.status(), `${path} should return 200`).toBe(200);
  ...
  121 | 
  122 |   test('hamburger is hidden on desktop', async ({ page }) => {
  123 |     await page.goto(`${BASE}/`);
  124 |     const toggle = page.locator('#mobile-menu-toggle');
  125 |     await expect(toggle).not.toBeVisible();
  126 |   });
  127 | 
  128 |   test('nav links are visible on desktop without clicking anything', async ({ page }) => {
  129 |     await page.goto(`${BASE}/`);
  130 |     const navLinks = page.locator('.nav-links');
  131 |     await expect(navLinks).toBeVisible();
  132 |   });
  133 | });
  134 | 
```