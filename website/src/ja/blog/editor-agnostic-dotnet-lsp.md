...
SharpLsp は、インストール済みの `sharplsp` バイナリ 1 つを中心に構築されています。エディタークライアントはそれを `PATH` から見つけ、標準入力/標準出力で起動します。同じサーバーが C#、F#、ソリューション検出、セマンティック要求、診断、SharpLsp 独自要求を処理します。
...
だからこそ、VS Code 拡張は Solution Explorer とプロファイラービューを提供でき、その一方で同じ `sharplsp` が標準 LSP 機能だけをサポートするエディターにもサービスを提供できます。
...
.NET エコシステムは、Windows や評価の低いファーストパーティ拡張、あるいは単一の商用 IDE ベンダーに縛られるには優れすぎています。
