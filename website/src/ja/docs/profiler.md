...
| ヒープ差分（50k 型のダンプ 2 つ） | <10s |
| グラフ webview 初期描画 | <500ms |

## エラー処理

すべてのプロファイラーコマンドはエラーを適切に処理します。

- **ツール未インストール**: 正確な `dotnet tool install` コマンド付きのエラーを返します
- **無効な PID**: LSP サーバーをクラッシュさせずにエラーを返します
- **ダンプファイル不足**: 明確なエラーメッセージを返します
- **セッション上限超過**: 最大同時セッション数に達した場合にエラーを返します
- **サイドカーから独立**: プロファイラーは完全に Rust ホスト内で動作し、サイドカーのクラッシュはプロファイリングに影響しません
