...
[VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=nimblesite.sharplsp) から SharpLsp 拡張機能をインストールします。拡張機能には `sharplsp` バイナリと両方のサイドカーが VSIX 内に同梱されています。Rust ツールチェーンや別途バイナリのインストールは不要です。
