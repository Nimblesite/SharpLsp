...
export async function fetchTargets(
    ...
    try {
        ...
        const result = await lsp.sendRequest<NuGetTargetsResponse>(
            "forge/nuget/targets",
            { workspaceRoot },
        );
        return ok(result);
...
export async function fetchInstalled(
    ...
    try {
        ...
        const result = await lsp.sendRequest<NuGetInstalledResponse>(
            ...
            { target, projectPath: target.path },
        );
        return ok(result);
    } catch (err: unknown) {
...
export async function searchPackages(
    ...
    try {
        log.info(
            `nuget/lsp: searchPackages target=${target.id} query="${query}"`,
        );
...
export async function installPackage(
    ...
    try {
        log.info(
            `nuget/lsp: install ${packageId} v${version} into ${target.id}`,
        );
        const result = await lsp.sendRequest<NuGetMutationResponse>(
            "forge/nuget/install",
            { target, projectPath: target.path, packageId, version },
        );
        return ok(result);
    } catch (err: unknown) {
        return fail(getErrorMessage(err));
...
export async function uninstallPackage(
    ...
    try {
        ...
        const result = await lsp.sendRequest<NuGetMutationResponse>(
            ...
            { target, projectPath: target.path, packageId },
        );
        return ok(result);
    } catch (err: unknown) {
        return fail(getErrorMessage(err));
