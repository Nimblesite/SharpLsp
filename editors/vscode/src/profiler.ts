...
function buildCounterHtml(counters: CounterValue[]): string {
    const rows = counters
        ...
        .map((c) => {
            const displayName =
                c.display_name.length > 0 ? c.display_name : c.name;
...
export class ProfilerStatusBar {
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        this.item.tooltip =
            "Active profiling sessions — click to list processes";
            ...
            async () => {
                ...
                try {
                    ...
                    if (
                        ...
                        !CounterWebviewPanel.has("__wired__")
                    ) {
        ...
        vscode.commands.registerCommand(
            ...
            async () => {
                ...
                try {
                    ...
                    void vscode.window.showInformationMessage(
                        "Counter monitoring stopped.",
                    );
                } catch (err: unknown) {
                    const msg = getErrorMessage(err);
                    void vscode.window.showErrorMessage(
                        `Stop counters failed: ${msg}`,
                    );
                }
            },
        ),
            ...
            async () => {
                ...
                try {
                    ...
                    for (const f of result.fields) {
                        const ref =
                            f.reference_address !== undefined
                                ? ` → ${f.reference_address}`
                                : "";
                        lines.push(
                            `  ${f.name}: ${f.type_name} = ${f.value}${ref}`,
                        );
                    }
