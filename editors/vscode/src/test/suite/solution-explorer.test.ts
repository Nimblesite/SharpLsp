...
EndGlobal`,
        ...
        assert.ok(
            symbols.length > 0,
            "LSP should return symbols for Calculator.cs",
        );
        ...
        const calcClass = nsSymbol.children?.find(
            (s) => s.name === "Calculator",
        );
        assert.ok(
            calcClass,
            "Should find Calculator class inside MyApp namespace",
        );
        ...
        assert.ok(
            iface,
            "Should find ICalculator interface inside MyApp namespace",
        );
...
public class ApiController
{
    ...
    public void Post() { }
        ...
        assert.ok(
            controller,
            "Should find ApiController class INSIDE the namespace",
        );
        ...
        assert.ok(
            rootClass === undefined ||
                rootClass.kind === vscode.SymbolKind.Namespace,
...
public interface ITransport
{
    void Send();
        ...
        assert.ok(
            envelope,
            "Envelope must be INSIDE Common.Messages namespace",
        );
        ...
        assert.ok(
            transport,
            "ITransport must be INSIDE Common.Messages namespace",
        );
...
public record UserDto(string Name, int Age);`;
        ...
        const controller = ns.children?.find(
            (s) => s.name === "HomeController",
        );
...
public class EventSource
{
    ...
    test("documentSymbol reflects unsaved edits (VFS-based, should pass)", async function () {
        ...
        const renamedClass = nsAfter.children?.find(
            (s) => s.name === "Renamed",
        );
        assert.ok(
            renamedClass,
