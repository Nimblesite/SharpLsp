...
suite("Install Module — getInstalledVersion()", () => {
    ...
    test("parses 'forge-lsp X.Y.Z' format correctly from real binary", function () {
        ...
        assert.ok(
            version !== undefined,
            `Expected a version string from ${binary}, got undefined`,
        );
        // Must be a valid semver-ish format.
        const segments = version.split(".");
        assert.ok(
            segments.length >= 2,
            `Version must have at least X.Y segments, got: ${version}`,
        );
    });
});
...
suite("Install Module — forge-lsp --version contract", () => {
    test("forge-lsp --version exits with code 0", function () {
        ...
        assert.strictEqual(
            result.status,
            0,
            `--version must exit 0, got ${String(result.status)}`,
        );
    });
    ...
    test("forge-lsp --version output is exactly 'forge-lsp X.Y.Z'", function () {
        ...
        assert.strictEqual(
            parts.length,
            2,
            `Expected 'forge-lsp X.Y.Z', got: ${trimmed}`,
        );
        ...
        assert.ok(
            segments.length >= 2,
            `Version must have at least X.Y, got: ${parts[1]}`,
        );
        for (const seg of segments) {
            assert.ok(
                /^\d+$/.test(seg),
                `Each version segment must be numeric, got '${seg}'`,
            );
        }
    });
    ...
    test("getInstalledVersion returns version matching --version output", function () {
        ...
        assert.strictEqual(
            parsed,
            expected,
            `getInstalledVersion must match raw --version output`,
        );
    });
});
...
suite("Install Module — version mismatch handling", () => {
    ...
    test("describeBinaryStatus reports found version for real binary", function () {
        ...
        assert.ok(
            ...
            `Must detect version from ${binary}`,
        );
    });
    ...
    test("describeBinaryStatus reports undefined for nonexistent path", () => {
        ...
        if (!fs.existsSync(status.location)) {
            assert.strictEqual(
                status.found,
                undefined,
                "Must report undefined when binary not found",
            );
        }
    });
