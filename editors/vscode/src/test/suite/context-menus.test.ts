...
suite("Context Menu — Package.json Contributions", () => {
    ...
    for (const cmd of [
        ...
        test(`${cmd} command is registered`, async () => {
            ...
            assert.ok(
                cmds.includes(cmd),
                `${cmd} must be a registered VS Code command`,
            );
        });
    }
    ...
    test("sortMembers menu entry exists in view/item/context", () => {
        const entry = menuEntries().find(
            (m) => m.command === "forge.sortMembers",
        );
        assert.ok(
            entry,
            "forge.sortMembers must have a view/item/context menu entry",
        );
    });
    ...
    test("sortMembers when clause scopes to class/struct/interface/enum/record", () => {
        ...
        assert.ok(
            when.includes("class"),
            `sortMembers when clause must include 'class': ${when}`,
        );
        assert.ok(
        ...
        assert.ok(
            when.includes("enum"),
            `sortMembers when clause must include 'enum': ${when}`,
        );
        assert.ok(
            when.includes("record"),
            `sortMembers when clause must include 'record': ${when}`,
        );
        assert.ok(
    ...
    test("sortMembers when clause does NOT include method or property", () => {
        ...
        assert.ok(
            !when.includes("symbol.method"),
            "sortMembers must NOT show for method nodes",
        );
        assert.ok(
            !when.includes("symbol.property"),
            "sortMembers must NOT show for property nodes",
        );
        assert.ok(
            !when.includes("symbol.field"),
            "sortMembers must NOT show for field nodes",
        );
    });
    ...
    test("copyQualifiedName menu entry exists", () => {
        const entry = menuEntries().find(
            (m) => m.command === "forge.copyQualifiedName",
        );
        assert.ok(
            entry,
            "forge.copyQualifiedName must have a view/item/context menu entry",
        );
    });
    ...
    test("copyName menu entry exists", () => {
        ...
        assert.ok(
            entry,
            "forge.copyName must have a view/item/context menu entry",
        );
    });
    ...
    test("copyName when clause covers symbol nodes", () => {
        ...
        assert.ok(
            entry.when.includes("symbol"),
            "copyName when clause must reference 'symbol'",
        );
        assert.ok(
            entry.when.includes("forge.solutionExplorer"),
    ...
    test("revealInExplorer menu entry exists", () => {
        const entry = menuEntries().find(
            (m) => m.command === "forge.revealInExplorer",
        );
        assert.ok(
            entry,
            "forge.revealInExplorer must have a view/item/context menu entry",
        );
    });
...
suite("Context Menu — Context Values on Tree Nodes", () => {
    ...
    test("project node has contextValue 'project'", () => {
        ...
        assert.ok(
            projectNode,
            "Project node must exist with contextValue 'project'",
        );
    });
    ...
    test("namespace node has contextValue 'symbol.namespace'", () => {
        const ns = findNode(
            ...
            (n) =>
                nodeLabel(n).includes("AllTypesNS") &&
                n.contextValue === "symbol.namespace",
        );
        assert.ok(
            ns,
            "AllTypesNS namespace node must have contextValue 'symbol.namespace'",
        );
    });
    ...
    test("interface node has contextValue 'symbol.interface'", () => {
        ...
        assert.ok(
            nodeLabel(node).includes("IRunner"),
            `Expected IRunner, got '${nodeLabel(node)}'`,
        );
    });
    ...
    test("struct node has contextValue 'symbol.struct'", () => {
        ...
        assert.ok(
            nodeLabel(node).includes("MyPoint"),
            `Expected MyPoint, got '${nodeLabel(node)}'`,
        );
    });
    ...
    test("enum node has contextValue 'symbol.enum'", () => {
        ...
        assert.ok(
            nodeLabel(node).includes("MyEnum"),
            `Expected MyEnum, got '${nodeLabel(node)}'`,
        );
    });
    ...
    test("enum member node has contextValue 'symbol.enumMember'", () => {
        ...
        assert.ok(
            node,
            "A symbol.enumMember node must exist (e.g., Alpha or Beta)",
        );
        ...
        const node = findNode(
            ...
            (n) =>
                ...
                (n.contextValue === "symbol.class" ||
                    n.contextValue === "symbol.record"),
        );
        assert.ok(
            node,
            "MyRecord must have contextValue 'symbol.class' or 'symbol.record'",
        );
    });
    ...
    test("all symbol nodes have a contextValue starting with 'symbol.'", () => {
        ...
        assert.ok(
            symbolCount > 0,
            "Must find at least some symbol nodes in the tree",
        );
        assert.deepEqual(
            badNodes,
            [],
            ...
            `namespace OuterNS
{
    ...
    test("copyQualifiedName for a class produces Namespace.ClassName", async function () {
        ...
        assert.ok(
            text.includes("."),
            "Qualified name must contain a dot separator",
        );
        assert.ok(
            !text.startsWith("."),
            "Qualified name must not start with a dot",
        );
    });
    ...
    test("copyQualifiedName for a method produces Namespace.Class.Method", async function () {
        ...
        const methodNode = findNode(
            provider.getChildren(),
            (n) =>
                nodeLabel(n).includes("OuterMethod") &&
                n.contextValue === "symbol.method",
        );
        ...
        const text = await vscode.env.clipboard.readText();
        assert.ok(
            text.includes("OuterNS") &&
                text.includes("OuterClass") &&
                text.includes("OuterMethod"),
    ...
    test("copyQualifiedName for a property produces Namespace.Class.Property", async function () {
        ...
        const propNode = findNode(
            provider.getChildren(),
            (n) =>
                nodeLabel(n).includes("OuterProp") &&
                n.contextValue === "symbol.property",
        );
    ...
    test("copyQualifiedName for inner class includes full path", async function () {
        ...
        const innerNode = findNode(
            provider.getChildren(),
            (n) =>
                nodeLabel(n).includes("InnerClass") &&
                n.contextValue === "symbol.class",
        );
    ...
    test("copyQualifiedName for inner method includes full hierarchy", async function () {
        ...
        const innerMethodNode = findNode(
            provider.getChildren(),
            (n) =>
                nodeLabel(n).includes("InnerMethod") &&
                n.contextValue === "symbol.method",
        );
        ...
        assert.ok(
            text.includes("."),
            "Inner method qualified name must have dots",
        );
    });
...
suite("Context Menu — Copy Name", () => {
    ...
    test("copyName for a class node copies unqualified class name", async function () {
        ...
        assert.strictEqual(
            text,
            "AllTypesClass",
            `Expected 'AllTypesClass', got '${text}'`,
        );
    });
    ...
    test("copyName does not include namespace prefix", async function () {
        ...
        assert.ok(
            !text.includes("."),
            `copyName must return unqualified name, got '${text}'`,
        );
    });
            ...
            `namespace RevealNS
{
    ...
    test("property node has symbolUri set", () => {
        const node = findNode(
            provider.getChildren(),
            (n) =>
                nodeLabel(n).includes("RevealProp") &&
                n.contextValue === "symbol.property",
        ) as (TreeNode & { symbolUri?: string }) | undefined;
        assert.ok(node, "RevealProp node must be in the tree");
        assert.ok(
            node.symbolUri !== undefined,
            "Property node must have symbolUri set",
        );
    });
    ...
    test("symbolUri on all symbol nodes is a valid file URI", () => {
        ...
        function walkNodes(nodes: TreeNode[] | undefined): void {
            ...
            for (const node of nodes) {
                ...
                if (cv.startsWith("symbol.") && cv !== "symbol.namespace") {
                    symbolCount++;
                    const uri = (node as TreeNode & { symbolUri?: string })
                        .symbolUri;
                    if (uri === undefined || uri === "") {
                        badNodes.push(`${nodeLabel(node)} (${cv})`);
                    } else if (!uri.startsWith("file://")) {
                        badNodes.push(
                            `${nodeLabel(node)} has non-file URI: ${uri}`,
                        );
                    }
                }
                walkNodes(node.children);
        ...
        assert.ok(
            symbolCount > 0,
            "Must find at least some non-namespace symbol nodes",
        );
        assert.deepEqual(
            badNodes,
            [],
...
suite("Context Menu — Correct Node Type Scoping", () => {
    test("sortMembers contextValue pattern matches type nodes only", () => {
        ...
        for (const cv of mustMatch) {
            assert.ok(
                pattern.test(cv),
                `sortMembers pattern must match '${cv}'`,
            );
        }
        for (const cv of mustNotMatch) {
            assert.ok(
                !pattern.test(cv),
                `sortMembers pattern must NOT match '${cv}'`,
            );
        }
    });
    ...
    test("copyQualifiedName contextValue pattern matches all symbol nodes", () => {
        ...
        for (const cv of mustMatch) {
            assert.ok(
                cv.startsWith(prefix),
                `copyQualifiedName must match '${cv}'`,
            );
        }
        assert.ok(
            !"solution".startsWith(prefix),
            "copyQualifiedName must not match 'solution'",
        );
        assert.ok(
            !"project".startsWith(prefix),
            "copyQualifiedName must not match 'project'",
        );
    });
    ...
    test("SYMBOL_CONTEXT_VALUES covers all expected symbol kinds", () => {
        ...
        assert.deepEqual(
            badKinds,
            [],
            "All symbol context values must start with 'symbol.'",
        );
    });
});
