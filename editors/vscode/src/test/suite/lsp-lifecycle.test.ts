...
suite("LSP Lifecycle", () => {
  ...
  test("forge.restartServer restarts the LSP server", async function () {
    ...
    assert.ok(
      symbols.length > 0,
      "Server should respond to symbols after restart",
    );
  });
  ...
  test("opening and closing multiple C# files works", async function () {
    ...
    assert.ok(
      flattenNames(symbols2).includes("Cycle2"),
  ...
  test("multiple files open simultaneously get independent symbols", async function () {
    ...
    const { uri: uriA } = await openCSharpFile(
      tmpDir,
      "simA.cs",
      "class Alpha { void X() { } }",
    );

    const { uri: uriB } = await openCSharpFile(
      tmpDir,
      "simB.cs",
      "class Beta { void Y() { } }",
    );
    ...
    assert.ok(
      !namesA.includes("Beta"),
      "File A should NOT contain Beta",
    );
    assert.ok(
      !namesB.includes("Alpha"),
      "File B should NOT contain Alpha",
    );
  });
  ...
  test("server handles rapid file open/close gracefully", async function () {
    ...
    assert.ok(
      symbols.length > 0,
      "Server should still respond after rapid open/close",
    );
  });
  ...
  test("restarting twice in succession does not crash", async function () {
    ...
    assert.ok(
      symbols.length > 0,
      "Server should respond after double restart",
    );
  });
  ...
  test("restart preserves ability to handle new files", async function () {
    ...
    assert.ok(
      names.includes("AfterRestart"),
      "New file after restart should be served",
    );
    assert.ok(
      names.includes("NewMethod"),
      "New file methods should be resolved",
    );
  });
