...
suite("Hover / Quick Info", () => {
  ...
  test("hover on class, method, property, field in one file", async function () {
    ...
    const content = `namespace HoverMulti
{
    ...
    const { uri } = await openCSharpFile(tmpDir, "HoverMulti.cs", content);
...
function assertNonSymbolNodesLackHoverData(nodes: TreeNode[]): void {
  for (const node of nodes) {
    if (node.nodeType === "solution" || node.nodeType === "project" ||
        node.nodeType === "dependencyFolder" || node.nodeType === "nugetPackage" ||
