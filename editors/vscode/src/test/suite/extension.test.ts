...
suite("Extension Activation & Configuration", () => {
  ...
  test("forge.showOutput command is registered", async () => {
    const allCommands = await vscode.commands.getCommands(true);
    assert.ok(
      allCommands.includes("forge.showOutput"),
      "forge.showOutput should be registered",
    );
  });
  ...
  test("forge.server.path setting is contributed", () => {
    ...
    assert.strictEqual(
      ...
      "",
      "Default server.path should be empty string",
    );
  });
  ...
  test("forge.server.extraArgs setting is contributed", () => {
    ...
    assert.deepStrictEqual(
      inspect.defaultValue,
      [],
      "Default extraArgs should be empty array",
    );
  });
  ...
  test("forge.trace.server setting is contributed", () => {
    ...
    assert.strictEqual(
      inspect.defaultValue,
      "off",
      "Default trace level should be off",
    );
  });
  ...
  test("csharp language contributes .cs extension", () => {
    ...
    assert.ok(csharp, "Should contribute csharp language");
    assert.ok(
      csharp.extensions?.includes(".cs"),
      "csharp should include .cs extension",
    );
  });
  ...
  test("csharp language contributes .csx extension", () => {
    ...
    assert.ok(csharp);
    assert.ok(
      csharp.extensions?.includes(".csx"),
      "csharp should include .csx extension",
    );
  });
  ...
  test("fsharp language contributes .fs extension", () => {
    ...
    assert.ok(fsharp);
    assert.ok(
      fsharp.extensions?.includes(".fs"),
      "fsharp should include .fs extension",
    );
  });
  ...
  test("fsharp language contributes .fsx extension", () => {
    ...
    assert.ok(fsharp);
    assert.ok(
      fsharp.extensions?.includes(".fsx"),
      "fsharp should include .fsx extension",
    );
  });
  ...
  test("fsharp language contributes .fsi extension", () => {
    ...
    assert.ok(fsharp);
    assert.ok(
      fsharp.extensions?.includes(".fsi"),
      "fsharp should include .fsi extension",
    );
  });
  ...
  test("extension has version string", () => {
    ...
    assert.ok(ext);
    assert.ok(
      typeof ext.packageJSON.version === "string",
      "version must be a string",
    );
    assert.match(ext.packageJSON.version, /^\d+\.\d+\.\d+/);
  });

  test("extension contributes 5 commands", () => {
    const ext = vscode.extensions.getExtension(EXTENSION_ID);
    assert.ok(ext);
    const cmds: { command: string }[] =
      ext.packageJSON.contributes?.commands ?? [];
    assert.strictEqual(cmds.length, 5, "Should contribute exactly 5 commands");
  ...
  test("extension has language configuration files", () => {
    ...
    for (const lang of languages) {
      assert.ok(
        lang.configuration,
        `Language ${lang.id} must have a configuration file`,
      );
      assert.ok(
