    ...
    const doRequest = (requestUrl: string, redirectCount: number): void => {
      ...
      const request = https.get(options, (response) => {
        ...
        if (response.statusCode !== 200) {
          reject(new Error(`Download failed: HTTP ${String(response.statusCode)}`));
          return;
        }
      ...
      request.on('timeout', () => {
        request.destroy(
          new Error(`Download timed out after ${String(DOWNLOAD_TIMEOUT_MS)}ms: ${requestUrl}`),
        );
      });
    };
...
async function downloadAndInstall(version: string): Promise<void> {
  ...
  void window.showInformationMessage(`Forge: Installing binaries v${version}...`);
  ...
  try {
    ...
    if (!fs.existsSync(binaryPath)) {
      throw new Error(`Installation completed but ${binaryPath} not found`);
    }
    ...
    void window.showInformationMessage(`Forge: Binaries v${version} installed to ${prefix}`);
  } finally {
    // Clean up temp files.
...
export async function ensureBinaries(configuredPath: string): Promise<InstallResult> {
  ...
  if (fs.existsSync(standardPath)) {
    ...
    log.info(`Installed binary version mismatch: expected ${version}, got ${String(installed)}`);
  }
  ...
  log.info(`Forge v${version} not found. Attempting download from GitHub releases.`);
