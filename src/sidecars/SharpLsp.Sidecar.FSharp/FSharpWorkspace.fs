...
/// Read explicit F# compiler flags from the project without forwarding MSBuild
/// property placeholders such as `$(OtherFlags)` to FCS.
let internal parseFsprojOtherFlags (fsprojPath: string) : string array =
    let separators = [| ' '; '\t'; '\r'; '\n' |]
    XDocument.Load(fsprojPath).Descendants(XName.Get("OtherFlags"))
    |> Seq.collect (fun element ->
        element.Value.Split(separators, StringSplitOptions.RemoveEmptyEntries))
...
let internal buildProjectOptions (state: FSharpWorkspaceState) (fsprojPath: string) : FSharpProjectOptions =
    ...
    let otherOptions =
        Array.concat [ frameworkReferenceArgs (); packageRefs; projectRefArgs; projectFlags ]
