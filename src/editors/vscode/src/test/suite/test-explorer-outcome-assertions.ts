...
export function assertPassed(result: CachedTestResult, id: string): void {
  ...
  assert.strictEqual(result.outcome, 'passed', `${id} passes in the fixture, so TRX must say so (${detail})`);
  assert.strictEqual(result.passed, true, `${id} passed, so the pass flag must be true`);
  assert.strictEqual(typeof result.duration, 'number', `${id} must carry the duration TRX recorded`);
  assert.strictEqual((result.duration ?? -1) >= 0, true, `${id} duration must be non-negative, got ${String(result.duration)}`);
  assert.strictEqual(result.message, undefined, `${id} passed, so no failure text may attach: ${result.message ?? 'none'}`);
  assert.strictEqual(statusLensTitle(result), `$(pass) Passed${formatDuration(result.duration)}`, `${id} renders as a pass carrying its own duration`);
}
...
export function assertFailed(result: CachedTestResult, id: string): void {
  assert.strictEqual(result.outcome, 'failed', `${id} fails in the fixture, got '${result.outcome}'`);
  assert.strictEqual(result.passed, false, `${id} failed, so the pass flag must be false`);
  assert.notStrictEqual(result.message, undefined, `${id} must carry the failure message TRX recorded`);
  const message = result.message ?? '';
  assert.strictEqual(message.includes('Assert.Equal() Failure'), true, `${id} must surface xUnit's own text: ${message}`);
  assert.strictEqual(message.includes('Expected'), true, `${id} must surface the Expected/Actual detail: ${message}`);
  assert.notStrictEqual(message, 'Test failed', `${id} must not fall back to the generic message`);
  assert.strictEqual(statusLensTitle(result), `$(error) Failed: ${message}`, `${id} renders its cached message verbatim`);
}
...
export function assertSkipped(result: CachedTestResult, id: string): void {
  assert.strictEqual(result.outcome, 'skipped', `${id} is [Fact(Skip=…)]: NotExecuted maps to 'skipped', got '${result.outcome}'`);
  assert.notStrictEqual(result.outcome, 'failed', `${id} is skipped and must NEVER be a failure — the original bug`);
  assert.strictEqual(result.passed, false, `${id} was not executed, so it is not a pass either`);
  const message = result.message ?? 'none';
  assert.strictEqual(message.includes('Assert'), false, `${id} is skipped, so no assertion text may attach: ${message}`);
  assert.strictEqual(statusLensTitle(result), '$(debug-step-over) Skipped', `${id} renders as a skip, never as a failure`);
}
