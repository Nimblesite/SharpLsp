...
suite('Test Explorer e2e — TRX and console readers on Windows shapes', () => {
  test('the TRX reader survives a BOM, CRLF, and every FQN shape Windows produces', function () {
    ...
    assert.strictEqual(
      results.length,
      6,
      `every <UnitTestResult> must be read back: got ${String(results.length)}`,
    );
    ...
    assert.strictEqual(
      pass.displayName,
      'Adds_TwoNumbers',
      'the display name is kept alongside the FQN',
    );
    assert.notStrictEqual(
      pass.displayName,
      pass.fullyQualifiedName,
      'display name and FQN are NOT the same value',
    );
    ...
    assert.ok(
      failure.message?.startsWith('Assert.Equal() Failure'),
      `the real assertion text must reach the user: ${String(failure.message)}`,
    );
    ...
    assert.ok(
      failure.stackTrace?.includes('Program Files (x86) copy'),
      'a stack trace through a hostile Windows path is preserved verbatim',
    );
    ...
    assert.ok(
      byName.has(FS_FACT_SPACED),
      `an F# name carrying SPACES must key verbatim: ${FS_FACT_SPACED}`,
    );
    assert.strictEqual(byName.get(FS_FACT_SPACED)?.outcome, 'passed', 'and report its own outcome');
    assert.ok(
      byName.has(NUNIT_CASE),
      `an NUnit case name carrying PARENTHESES must key verbatim: ${NUNIT_CASE}`,
    );
    assert.strictEqual(
      byName.get(NUNIT_CASE)?.displayName,
      'Adds_Case(2,2,4)',
      "NUnit's display name is the bare method plus its row",
    );
    assert.ok(
      byName.has(FS_MSTEST_NESTED),
      `an F# MSTest class is a CLR NESTED type, so its FQN carries '+': ${FS_MSTEST_NESTED}`,
    );
    assert.strictEqual(
      byName.get(FS_MSTEST_NESTED)?.displayName,
      'AddsTwoNumbers',
      'MSTest renders only the bare member name',
    );
    assert.strictEqual(
      byName.get(FS_MSTEST_NESTED)?.durationMs,
      1500,
      '00:00:01.5000000 is 1500ms',
    );
    ...
    assert.strictEqual(
      parseTrx(orphanTrx('Passed'))[0]?.fullyQualifiedName,
      'Orphan',
      'an orphaned result keeps its display name as the key',
    );
    ...
    assert.deepStrictEqual(
      parseTrx('<not-trx/>'),
      [],
      'a document that is not a TRX yields no results',
    );
    assert.deepStrictEqual(
      parseTrx('<TestRun></TestRun>'),
      [],
      'a TRX with no results yields none',
    );
  ...
  test('a REFUSED filter and an unmatched filter are told apart by the run info', function () {
    ...
    assert.strictEqual(
      ...
      0,
      'a refused filter produces no test results at all',
    );
    ...
    assert.ok(
      warning.text.includes('No test matches'),
      `VSTest's own words are kept: ${warning.text}`,
    );
    ...
    assert.deepStrictEqual(
      ...
      [],
      'a non-TRX document yields no run info',
    );
  });
  ...
  test('the console reader sums every assembly and never calls a skip a failure', function () {
    ...
    assert.deepStrictEqual(
      {
        passed: summed.passed,
        failed: summed.failed,
    ...
    assert.strictEqual(
      summed.outcome,
      'failed',
      'one failing assembly makes the whole run a failure',
    );
    assert.strictEqual(
      summed.total,
      summed.passed + summed.failed + summed.skipped,
      'the totals are self-consistent',
    );

    // CRLF is what Windows writes.
    assert.deepStrictEqual(
      parseRunSummary(solution.split('\r\n').join('\n')),
      summed,
      'CRLF and LF parse identically',
    );
  ...
  test('the dotnet child process is pinned to English and sized for a cold Windows restore', function () {
    ...
    assert.strictEqual(
      DOTNET_CLI_LANGUAGE,
      'en-US',
      'the CLI UI language is pinned so the parsed strings are deterministic',
    );
    assert.strictEqual(
      typeof DOTNET_CLI_LANGUAGE,
      'string',
      'and it is a plain string, passed straight into the child env',
    );
  ...
  test('MSBuild percent-escaping in the assembly banner is decoded, not dropped', function () {
    ...
    assert.deepStrictEqual(
      ...
      [escaped],
      'the parser reports the banner verbatim, escaping and all',
    );
    assert.deepStrictEqual(
      parseAnnouncedAssemblies(`${banner}\r\n${banner}`),
      [escaped],
      'a repeated CRLF banner yields one entry',
    );
    assert.deepStrictEqual(
      parseTestAssemblies(banner),
      [],
      'and neither spelling exists on THIS disk, so nothing survives the filter',
    );
    assert.strictEqual(
      resolveAnnouncedAssembly(escaped),
      undefined,
      'an assembly that exists in neither spelling resolves to undefined',
    );
  });
});
