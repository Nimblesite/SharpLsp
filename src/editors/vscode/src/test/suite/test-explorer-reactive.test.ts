...
import { XUNIT_PACKAGES, createSolution, dotnet, projectXml, warmDiscovery, writeProject } from './dotnet-project-kit';
import { fixtureFor } from './test-explorer-fixtures';
import { activateTestExplorer, collectItemIds, discoverSolution, drainDiscovery, findItem, pollForIds, pollUntilDiscovered, snapshotItems } from './test-explorer-kit';
...
function fromController(stack: string): boolean {
  return !stack.includes(THIS_FILE) && (stack.includes('extension.js') || stack.includes('testing'));
}
...
function spyOnDiscover(controller: SharpLspTestController): DiscoverSpy {
  ...
  return { stacks, restore: (): void => { Reflect.deleteProperty(controller, 'discover'); } };
}
...
function assertTree(ctl: SharpLspTestController, expected: readonly string[], why: string): string[] {
  ...
  assert.strictEqual(ctl.items.size, expected.length, `${why}: the root holds each test exactly once`);
  assert.deepStrictEqual(collectItemIds(ctl.items), ids, `${why}: two reads of a settled tree must agree`);
  return ids;
}
...
function assertRows(controller: SharpLspTestController, solutionDir: string, why: string): void {
  ...
  for (const row of snapshots) {
    ...
    assert.strictEqual(comparablePath(row.uriPath ?? ''), comparablePath(solutionDir), `${why}: ${row.id} anchored`);
    assert.strictEqual(row.childCount, 0, `${why}: discovery renders a flat list`);
    assert.deepStrictEqual(row.tags, [], `${why}: ${row.id} — a plain xUnit test carries no framework tag`);
  }
}
...
function assertFqnShape(ids: readonly string[], why: string): void {
  for (const id of ids) {
    ...
    assert.strictEqual(id.includes('.'), true, `${why}: '${id}' is not a dotted fully-qualified name`);
    assert.strictEqual(id.includes('('), false, `${why}: '${id}' — an xUnit theory FQN carries no row data`);
    assert.strictEqual(id.startsWith('Cs.'), true, `${why}: '${id}' is outside the fixture's namespaces`);
  }
...
function assertLeaf(ctl: SharpLspTestController, id: string, why: string): vscode.TestItem {
  ...
  assert.deepStrictEqual(item.tags.map((tag) => tag.id), [], `${why}: xUnit C# earns no 'fsharp' tag`);
  return item;
}

function assertCacheUntouched(ctl: SharpLspTestController, before: number, why: string): void {
  assert.strictEqual(ctl.cachedResults.size, before, `${why}: a sweep must not fabricate run results`);
  assert.strictEqual(ctl.getResult(ADDED_FQN), undefined, `${why}: a test that never ran has no outcome`);
}
...
suite('Test Explorer e2e — reactive discovery, refresh and tree lifecycle', () => {
  ...
  suiteSetup(async function () {
    ...
    assert.strictEqual(CS.mixedParameterized, CS_MIXED_THEORY, 'the fixture still exposes the mixed theory');
    assert.strictEqual(new Set(EXPECTED).size, EXPECTED.length, 'EXPECTED itself must be a set, not a bag');
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'sharplsp-reactive-'));
    const csDir = path.join(root, CS.projectName);
    csProjDir = writeProject(csDir, CS.projectFileName, projectXml(CS.packages), CS.sourceFileName, CS.source);
    ...
    extraProjDir = writeProject(extraDir, `${EXTRA_PROJECT}.csproj`, extraXml, 'ExtraTests.cs', EXTRA_SOURCE);
  ...
  teardown(async function () {
    ...
    await drainDiscovery(() => { api.explorerProvider.clear(); }, api.testController);
  });
  ...
  test('loading a solution repopulates the tree with no discover() call from this test', async function () {
    ...
    assert.strictEqual(new Set(baseline).size, baseline.length, 'the baseline itself carries no duplicates');
    assert.strictEqual(baseline.includes(CS_MIXED_THEORY), true, 'including the theory whose rows disagree');
    ...
    try {
      api.testController.items.replace([]);
      api.explorerProvider.clear();
      assert.deepStrictEqual(collectItemIds(api.testController.items), [], 'empty before the reactive load');
      ...
      assert.deepStrictEqual(sorted(ids), sorted(EXPECTED), 'the poll observed the whole fixture, reactively');
      ...
      assert.strictEqual(api.testController.items.get(CS.failing)?.id, CS.failing, 'the root resolves by FQN id');
      assert.strictEqual(spy.stacks.length >= 1, true, 'the subscription must have run at least one sweep');
      assert.strictEqual(typeof spy.stacks[0], 'string', 'and the spy recorded that sweep\'s call site');
      assert.strictEqual(spy.stacks.every(fromController), true, `not the controller's own:\n${spy.stacks.join('\n')}`);
      assert.strictEqual(spy.stacks.join('\n').includes(THIS_FILE), false, 'no sweep was requested by this test');
      assertCacheUntouched(api.testController, cached, 'reactive reload');
    } finally {
      spy.restore();
    }
    assert.strictEqual(Object.getOwnPropertyDescriptor(api.testController, 'discover'), undefined, 'spy restored');
    assert.strictEqual(typeof api.testController.discover, 'function', 'the real discover() is back in place');
  ...
  test('a burst of solution changes coalesces into exactly one discovery sweep', async function () {
    ...
    try {
      ...
      await drainDiscovery(() => undefined, api.testController);
      assert.strictEqual(spy.stacks.length, 1, `six signal writes must debounce to ONE sweep, saw ${spy.stacks.length}`);
      assert.strictEqual(spy.stacks[0]?.includes(THIS_FILE), false, 'and that sweep is the controller\'s own');
      assert.strictEqual(spy.stacks.every(fromController), true, 'and it ran inside the extension, not this test');
      const ids = await pollUntilDiscovered(api.testController, EXPECTED);
      assert.deepStrictEqual(sorted(ids), sorted(EXPECTED), 'the burst leaves exactly the fixture behind');
      assert.strictEqual(ids.filter((id) => id === CS.parameterized).length, 1, 'both [InlineData] rows collapse to ONE item');
      assert.strictEqual(ids.filter((id) => id === CS_MIXED_THEORY).length, 1, 'a theory whose rows disagree is one item too');
      assert.strictEqual(new Set(ids).size, EXPECTED.length, 'the burst produced one tree, not two');
      assert.strictEqual(ids.length, EXPECTED.length, 'and appended nothing to it');
      assert.strictEqual(ids.includes(CS.failing), true, 'a failing test is discovered like any other');
      ...
      assert.strictEqual(findItem(api.testController.items, CS.passing)?.id, CS.passing, 'resolves by FQN id');
      assertCacheUntouched(api.testController, cached, 'settled tree after a coalesced burst');
      assert.strictEqual(spy.stacks.length, 1, 'and no further sweep ran while the tree was being read');
    } finally {
      spy.restore();
    }
    assert.strictEqual(Object.getOwnPropertyDescriptor(api.testController, 'discover'), undefined, 'spy restored');
  ...
  test('adding a [Fact] on disk adds exactly one item, and removing it undoes that', async function () {
    ...
    assert.strictEqual(editedText.split('\n').length, originalText.split('\n').length + 1, 'added exactly one line');
    ...
    assert.strictEqual(grown.includes(ADDED_FQN), true, `the new test must appear: ${ADDED_FQN}\ngot: ${grown.join(', ')}`);
    assert.strictEqual(grown.length, before.length + 1, 'the tree grew by exactly one');
    assert.deepStrictEqual(sorted(grown), sorted([...EXPECTED, ADDED_FQN]), 'and by exactly that one name');
    ...
    assert.strictEqual(added.label, 'Adds_Reactively', 'the label is the method name the user typed');
    assert.strictEqual(comparablePath(added.uri?.fsPath ?? ''), comparablePath(solutionDir), 'anchored at the solution');
    // …and the user deletes it again.
    fs.writeFileSync(csSourcePath, originalText, 'utf8');
    assert.strictEqual(fs.readFileSync(csSourcePath, 'utf8'), CS.source, 'the fixture source is restored byte for byte');
    await vscode.commands.executeCommand('testing.refreshTests');
    const shrunk = await pollForIds(api.testController, (ids) => !ids.includes(ADDED_FQN));
    assert.strictEqual(shrunk.includes(ADDED_FQN), false, `a deleted test must leave: ${ADDED_FQN}\ngot: ${shrunk.join(', ')}`);
    assert.strictEqual(findItem(api.testController.items, ADDED_FQN), undefined, 'and must no longer resolve by id');
    assert.strictEqual(api.testController.items.get(ADDED_FQN), undefined, 'nor be reachable from the root collection');
  ...
  test('a project joining and leaving the solution adds and removes its tests', async function () {
    ...
    assert.strictEqual(listed.includes(`${EXTRA_PROJECT}.csproj`), true, 'the CLI reports the added project');
    assert.strictEqual(listed.includes(CS.projectFileName), true, 'and still reports the original one');
    assert.strictEqual(fs.readFileSync(slnPath, 'utf8').includes(`${EXTRA_PROJECT}.csproj`), true, 'the sln carries it');
    await vscode.commands.executeCommand('testing.refreshTests');
    const both = await pollForIds(api.testController, (ids) => EXTRA_TESTS.every((n) => ids.includes(n)));
    assert.strictEqual(both.length, EXPECTED.length + EXTRA_TESTS.length, 'both projects, in full');
    assert.deepStrictEqual(sorted(both), sorted([...EXPECTED, ...EXTRA_TESTS]), 'exactly the two projects\' tests');
    ...
    assert.strictEqual(partition('Cs.Extra.Fixtures.'), EXTRA_TESTS.length, 'the joined project contributes both tests');
    assert.strictEqual(partition('Cs.Xunit.Fixtures.'), EXPECTED.length, 'and the original keeps all of its own');
    ...
    assert.strictEqual(joined.label, 'Multiplies_TwoNumbers', 'the joined project\'s row renders its method name');
    assert.notStrictEqual(comparablePath(csProjDir), comparablePath(extraProjDir), 'the projects are distinct dirs');
    assert.strictEqual(comparablePath(joined.uri?.fsPath ?? ''), comparablePath(solutionDir), 'anchored at the solution');
    assert.notStrictEqual(comparablePath(joined.uri?.fsPath ?? ''), comparablePath(extraProjDir), 'not at its own project');
    ...
    assert.strictEqual(listedAfter.includes(`${EXTRA_PROJECT}.csproj`), false, 'the CLI no longer reports it');
    assert.strictEqual(listedAfter.includes(CS.projectFileName), true, 'while the original project stays');
    assert.strictEqual(fs.readFileSync(slnPath, 'utf8').includes(`${EXTRA_PROJECT}.csproj`), false, 'the sln dropped it');
    await vscode.commands.executeCommand('testing.refreshTests');
    const after = await pollForIds(api.testController, (ids) => !EXTRA_TESTS.some((n) => ids.includes(n)));
    for (const extra of EXTRA_TESTS) {
      assert.strictEqual(after.includes(extra), false, `removed project's tests must go: ${extra}\n${after.join()}`);
    }
    ...
    assert.strictEqual(fs.existsSync(path.join(extraProjDir, 'ExtraTests.cs')), true, 'the project is still on disk');
  ...
  test('a discovery sweep where every target fails keeps the last good tree', async function () {
    ...
    await assert.doesNotReject(async () => api.testController.discover(), 'a missing target must not reject');
    await drainDiscovery(() => undefined, api.testController);
    ...
    assert.strictEqual(kept.includes(CS_MIXED_THEORY), true, 'the kept tree keeps even the mixed theory');
    assert.strictEqual(api.testController.items.size, good.length, 'and the root collection kept its rows');
    ...
    const survivor = assertLeaf(api.testController, CS.passing, 'tree kept across a total discovery failure');
    ...
    assert.deepStrictEqual(sorted(recovered), sorted(EXPECTED), 'and rediscovers exactly the fixture');
    assert.strictEqual(recovered.includes(CS.skipped), true, 'including the skipped test the sweep lost');
  ...
  test('whenIdle() really drains the dotnet queue before it resolves', async function () {
    ...
    assert.strictEqual(idleMs < 1_000, true, `a drained queue must settle at once, took ${idleMs}ms`);
    ...
    assert.strictEqual(new Set(settled).size, settled.length, 'a drained tree carries no duplicates');
    assert.strictEqual(settled.includes(CS.parameterized), true, 'a theory is one drained item, not two');
    assert.strictEqual(findItem(api.testController.items, CS.skipped)?.label, 'Skipped_OnPurpose', 'label kept');
    ...
    await assert.doesNotReject(async () => api.testController.discover(), 'a sweep after whenIdle() must not reject');
    const sweepMs = Date.now() - sweepStart;
    assert.strictEqual(sweepMs < 240_000, true, `a warm sweep must finish inside its window, took ${sweepMs}ms`);
    const secondIdleStart = Date.now();
    await api.testController.whenIdle();
    const secondIdleMs = Date.now() - secondIdleStart;
    assert.strictEqual(secondIdleMs < 1_000, true, `whenIdle() must resolve at once after its sweep, ${secondIdleMs}ms`);
    assertTree(api.testController, EXPECTED, 'the tree is unchanged by an idempotent re-sweep');
    assert.deepStrictEqual(sorted(collectItemIds(api.testController.items)), sorted(settled), 'same as the drained read');
    assert.strictEqual(api.testController.items.size, EXPECTED.length, 'the root count agrees');
    assert.strictEqual(new Set(collectItemIds(api.testController.items)).size, EXPECTED.length, 'replaced, not appended');
    assert.strictEqual(findItem(api.testController.items, CS.failing)?.description, CS.failing, 'the FQN is kept');
  ...
  test('revealing the Testing view leaves a populated tree behind the refresh affordance', async function () {
    ...
    assert.strictEqual(commands.includes('testing.refreshTests'), true, 'the workbench exposes the ⟳ refresh command');
    assert.strictEqual(commands.includes('workbench.view.testing.focus'), true, 'the workbench reveals the Testing view');
    assert.strictEqual(commands.filter((id) => id === 'testing.refreshTests').length, 1, 'registered exactly once');
    await api.explorerProvider.loadSolution(slnPath);
    const reveal = async (): Promise<unknown> => vscode.commands.executeCommand('workbench.view.testing.focus');
    ...
    const ids = await pollUntilDiscovered(api.testController, EXPECTED);
    assert.deepStrictEqual(sorted(ids), sorted(EXPECTED), 'revealing the view must leave the fixture on screen');
    ...
    assert.strictEqual(snapshotItems(api.testController.items).length, EXPECTED.length, 'one rendered row per test');
    // The buttons the revealed view draws come from the registered profiles.
    const profiles = api.testController.profiles;
    assert.strictEqual(profiles.length, 3, 'Run, Debug and Coverage are the three registered profiles');
    assert.deepStrictEqual(profiles.map((p) => p.label), ['Run', 'Debug', 'Run with Coverage'], 'in the order shown');
    assert.strictEqual(new Set(profiles.map((p) => p.label)).size, 3, 'each profile is registered exactly once');
    const kinds = [vscode.TestRunProfileKind.Run, vscode.TestRunProfileKind.Debug, vscode.TestRunProfileKind.Coverage];
    assert.deepStrictEqual(profiles.map((p) => p.kind), kinds, 'each profile keeps the kind its button binds to');
    // And the affordance still works after the reveal.
    const refresh = async (): Promise<unknown> => vscode.commands.executeCommand('testing.refreshTests');
    ...
    assert.deepStrictEqual(sorted(refreshed), sorted(EXPECTED), 'refresh after a reveal repopulates the same set');
    assert.strictEqual(refreshed.includes(CS.skipped), true, 'a skipped test is still a discovered test');
    assert.strictEqual(refreshed.length, EXPECTED.length, 'and refresh replaced the tree rather than growing it');
    assert.strictEqual(api.testController.items.size, EXPECTED.length, 'the root holds each once');
  ...
  test('two concurrent activateAndDiscover() sweeps settle on one duplicate-free tree', async function () {
    ...
    await api.explorerProvider.loadSolution(slnPath);
    api.testController.items.replace([]);
    assert.deepStrictEqual(collectItemIds(api.testController.items), [], 'the race starts from an empty tree');
    ...
    await drainDiscovery(() => undefined, api.testController);
    const ids = await pollUntilDiscovered(api.testController, EXPECTED);
    assert.deepStrictEqual(sorted(ids), sorted(EXPECTED), 'the race leaves exactly one copy of the fixture');
    ...
    assert.strictEqual(ids.includes(CS_MIXED_THEORY), true, 'the mixed theory survives the race exactly once');
    ...
    assert.strictEqual(read().length, EXPECTED.length, 'the settled read is the whole fixture, once');
    for (const expected of EXPECTED) {
      assertLeaf(api.testController, expected, 'tree after two concurrent sweeps');
      assert.strictEqual(api.testController.items.get(expected)?.id, expected, 'and is reachable from the root');
    }
    await assert.doesNotReject(async () => api.testController.whenIdle(), 'the queue must drain cleanly');
