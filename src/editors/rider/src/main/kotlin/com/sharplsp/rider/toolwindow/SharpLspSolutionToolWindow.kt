...
import com.sharplsp.rider.toolwindow.nodes.SharpLspTreeNode
import com.sharplsp.rider.toolwindow.nodes.SolutionRootNode
...
/**
 * The SharpLsp Solution tool window.
 *
 ...
 * [SharpLspTreeNode] from the `nodes` package — the node objects encode
 ...
 */
class SharpLspSolutionToolWindow(
    ...
    private fun configureTree(tree: Tree) {
        ...
        tree.cellRenderer = SharpLspTreeCellRenderer()
            ...
            object : com.intellij.openapi.actionSystem.ActionGroup() {
                override fun getChildren(e: com.intellij.openapi.actionSystem.AnActionEvent?): Array<com.intellij.openapi.actionSystem.AnAction> {
                    ...
                    val mutable = path.lastPathComponent as? DefaultMutableTreeNode
                        ?: return emptyArray()
                    return SharpLspTreeActions.menuFor(project, mutable).childActionsOrStubs
                }
            },
            "SharpLspSolutionToolWindow",
        )
    ...
    /**
     * Drive a node's children from its [SharpLspTreeNode].
     *
    ...
    private fun loadChildren(mutable: DefaultMutableTreeNode) {
        val sharplsp = mutable.userObject as? SharpLspTreeNode ?: return
        if (sharplsp.childrenLoaded) return
        sharplsp.childrenLoaded = true
        ...
        sharplsp.loadChildren(project) { children ->
            SwingUtilities.invokeLater {
                mutable.removeAllChildren()
                for (child in children) {
                    mutable.add(wrapSharpLspNode(child))
                }
                treeModel.nodeStructureChanged(mutable)
            }
        }
    ...
    /**
     * Wrap a [SharpLspTreeNode] in a Swing [DefaultMutableTreeNode]. If the
     ...
     */
    private fun wrapSharpLspNode(node: SharpLspTreeNode): DefaultMutableTreeNode {
    ...
    private fun handleActivation(path: TreePath) {
        ...
        val sharplsp = mutable.userObject as? SharpLspTreeNode ?: return
        val target = sharplsp.navigationTarget() ?: return
    ...
    private inner class RefreshAction : AnAction(
        "Refresh",
        "Reload the SharpLsp solution tree",
        com.intellij.icons.AllIcons.Actions.Refresh,
    ) {
        override fun actionPerformed(e: AnActionEvent) {
            val sharplsp = rootNode.userObject as? SharpLspTreeNode ?: return
            sharplsp.childrenLoaded = false
            loadChildren(rootNode)
        }
    }
    ...
    private fun subscribeToVfsChanges() {
        ...
        connection.subscribe(VirtualFileManager.VFS_CHANGES, object : BulkFileListener {
            override fun after(events: List<VFileEvent>) {
                ...
                if (!relevant) return
                SwingUtilities.invokeLater {
                    val sharplsp = rootNode.userObject as? SharpLspTreeNode ?: return@invokeLater
                    sharplsp.childrenLoaded = false
                    loadChildren(rootNode)
                }
            }
 ...
 *
 * Defers all presentation to `SharpLspTreeNode.render(...)`, which sets
 ...
 */
private class SharpLspTreeCellRenderer : com.intellij.ui.ColoredTreeCellRenderer() {
...
/** Data class used by `SharpLspTreeNode.navigationTarget()`. */
