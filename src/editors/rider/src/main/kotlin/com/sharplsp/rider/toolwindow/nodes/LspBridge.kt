...
import com.sharplsp.rider.lsp.SharpLsp4jServer
import com.sharplsp.rider.lsp.SharpLspServerDescriptor
import com.sharplsp.rider.lsp.SharpLspServerSupportProvider
import java.util.concurrent.CompletableFuture

/**
 * Glue for talking to a running sharplsp instance from tree nodes.
 *
...
object LspBridge {
    ...
    /**
     * Return the live sharplsp server for this project, starting one if
    ...
    fun server(project: Project): LspServer? {
        ...
        val running = mgr.getServersForProvider(SharpLspServerSupportProvider::class.java)
            .firstOrNull { it.state == LspServerState.Running }
        if (running != null) return running

        log.info("sharplsp not running; starting it for project ${project.name}")
        try {
            mgr.ensureServerStarted(
                SharpLspServerSupportProvider::class.java,
                SharpLspServerDescriptor(project),
            )
        } catch (err: Throwable) {
            log.warn("failed to start sharplsp", err)
            return null
        }

        val deadline = System.currentTimeMillis() + SERVER_START_TIMEOUT_MS
        while (System.currentTimeMillis() < deadline) {
            val servers = mgr.getServersForProvider(SharpLspServerSupportProvider::class.java)
            val ready = servers.firstOrNull { it.state == LspServerState.Running }
            if (ready != null) {
                log.info("sharplsp reached Running state")
                return ready
            }
            ...
            if (dead != null) {
                log.warn("sharplsp start failed: state=${dead.state}")
                return null
            }
            Thread.sleep(SERVER_POLL_INTERVAL_MS)
        }
        log.warn("sharplsp did not reach Running state within ${SERVER_START_TIMEOUT_MS}ms")
        return null
    }
     ...
     * timeout — the long-lived `sharplsp/workspaceSymbols` call on a big
     * solution can take several seconds on a cold start.
     */
    fun <T> call(
        project: Project,
        timeoutMs: Int = 30_000,
        block: (SharpLsp4jServer) -> CompletableFuture<T>,
    ): CompletableFuture<T> {
            ...
            try {
                ...
                if (srv == null) {
                    result.completeExceptionally(
                        IllegalStateException("sharplsp is not running for this project"),
                    )
                    return@executeOnPooledThread
                }
                val value: T? = srv.sendRequestSync(timeoutMs) { lsp4j ->
                    @Suppress("UNCHECKED_CAST")
                    block(lsp4j as SharpLsp4jServer)
                }
                if (value == null) {
                    result.completeExceptionally(
                        IllegalStateException("sharplsp returned no response (timeout or closed)"),
                    )
                } else {
                    result.complete(value)
                }
            } catch (err: Throwable) {
                log.warn("sharplsp custom request failed", err)
