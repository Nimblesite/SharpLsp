...
module Forge.Sidecar.FSharp.Tests.SidecarEndToEndTests
...
open Xunit
open MessagePack
open Forge.Sidecar.Common.Ipc
open Forge.Sidecar.Common.Messages
...
let private createTestProject () =
    let dir = Path.Combine(Path.GetTempPath(), $"forge-e2e-{Guid.NewGuid():N}")
...
type SidecarFixture() =
    ...
    let sock = Path.Combine(Path.GetTempPath(), $"forge-e2e-{Guid.NewGuid():N}.sock")
...
let ``loadProject with missing fsproj returns error`` () = task {
    let st = FSharpWorkspace.create ()
    let empty = Path.Combine(Path.GetTempPath(), $"forge-empty-{Guid.NewGuid():N}")
