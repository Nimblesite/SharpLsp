...
- [x] Version-gate the push pipeline ([DIAG-PUSH-GATE](../specs/DIAGNOSTICS-SPEC.md#13-diag-push-gate-push-convergence-guarantee)): per-URI push generations, stale results never published, failed fetch for the newest generation retried until published or superseded — fixes GitHub #160 (phantom F# error survived a revert when the one post-revert fetch was dropped). Rust test: `failed_fetch_after_revert_must_not_strand_stale_published_diagnostics`; F# sidecar guard: `diagnostics clear after an error edit is reverted`
