...
### B12. An edited or added data row is skipped by the run, and a red row reports green (High) — **FOUND LATER, FIXED**

> Found by the new F# suite (`test-explorer-mtp-fsharp.test.ts`), which edits an F# `[<Theory>]` row between discovery and ▶. The row was reported **passed** although it fails. A `--filter-uid` value is the module's own key. Verified against real modules: `xunit.v3` hashes a row's arguments into its uid (`971f…` became `4114…` when `InlineData(2, 2, 4)` became `(2, 2, 5)`), and MSTest keys a row by position, so an ADDED row gets a uid too. The run rebuilt the module (B1) but still filtered it by the uids discovery had seen, so the edited or added row never ran, and the test was judged only by its untouched rows. Failing tests first: `test-explorer-mtp-fsharp.test.ts` ("an edited F# binding is REBUILT…", red: `adds rows … got 'passed'`) and `test-explorer-mtp-rows.test.ts` (C# `xunit.v3` and MSTest, rows added and edited; both tests red with the fix reverted: `Adds_Rows fails in the fixture, got 'passed'`). Fixed in `test-mtp-run.ts` and `test-mtp-discovery.ts` (`relistModule`): after the build, a filtered run lists each module again and selects by the uids that build reports ([TEST-MTP-RUN]).
